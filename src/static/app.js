        ...
        const participantsList = details.participants
          .map(participant => `
            <li class="participant-item">
              ...
              <button class="remove-btn" data-activity="${name}" data-email="${participant}" aria-label="Remove ${participant}">✖</button>
            </li>`)
          .join("");
  ...
  signupForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // keep the same class pattern used elsewhere
  ...
  activitiesList.addEventListener("click", async (event) => {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        // Re-fetch activities to keep UI consistent
        fetchActivities();
      } else {
        messageDiv.textContent = result.detail || "An error occurred";
